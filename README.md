...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv acheck` or `python -m venv acheck-venv`
...
- [ ] Make sure the virtualenv is activated `workon acheck` or `source acheck-venv/bin/activate`
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `acheck/backend/acheck/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `acheck/backend/acheck/settings/production.py`
...
- Create a copy of ``acheck/settings/local.py.example``:  
  `cp acheck/settings/local.py.example acheck/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/acheck/settings/base.py``.
- Adjust ``backend/acheck/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
...
- `SENTRY_API_KEY` - Sentry APIInfo key that needs to be generated on Sentry. [You can find or create authentication tokens within Sentry](https://sentry.io/api/).
